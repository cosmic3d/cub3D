...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/15 14:12:21 by apresas-          #+#    #+#             */
/*   Updated: 2024/04/15 14:28:02 by apresas-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	bonus_init_raycasting(t_data *data)
{
	int		x;
	t_img	*texture;
	t_img	*window;
...
void check_hit(t_data *data)
{
	while (data->ray.hit == 0)
	{
		...
		if (data->map.grid[data->ray.map[Y]][data->ray.map[X]] == WALL ||
			data->map.grid[data->ray.map[Y]][data->ray.map[X]] == DOOR) //LOL
			data->ray.hit = 1;
	}
}
